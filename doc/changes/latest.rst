...
- :bdg-primary:`Doc` Add typehints for parameters in :func:`~image.math_img` (:gh:`6170` by `Maximilian Cosmo Sitter`_)
...
- :bdg-success:`API` Change to raise TypeError and ValueError instead of KeyError when invalid arg is passed to ``copy_header_from`` in :func:`~image.math_img` (:gh:`6170` by `Maximilian Cosmo Sitter`_).
